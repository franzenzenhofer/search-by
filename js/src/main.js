...
(function() {
  ...
  $.each(colors, function() {
    return $('#colors').append("<a class='tool color' href='#searchcanvas' data-color='#" + this + "' style='background: #" + this + ";'>&nbsp;</a> ");
  });

  $.each([3, 6, 8, 10, 14, 18, 24, 27, 30, 33, 36, 40, 45, 50], function() {
    var glow;
    glow = '';
    if (this.toString() === "33") {
      glow = 'glow';
    }
    return $('#sizes').append("<a class='tool size " + glow + "' href='#searchcanvas' data-size='" + this + "' style='background: black; height: " + this + "px; width: " + this + "px;'>&nbsp;</a>");
  });

  $('.size').click(function(elem) {
    $('.size').each(function(elem) {
      return $(this).removeClass('glow');
    });
    return $(this).addClass('glow');
  });

  $('.color').click(function(elem) {
    var c;
    c = $(this).attr('data-color');
    return $('.size').css('background', c);
  });
